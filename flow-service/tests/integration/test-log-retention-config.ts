...
async function testLogRetentionConfig() {
  ...
  try {
    ...
    console.log(
      '  - Rotation Interval:',
      fileChannel['fsvc:logRotationInterval'],
    );
    ...
    if (envConfig['fsvc:hasLoggingConfig']) {
      const envFileChannel =
        envConfig['fsvc:hasLoggingConfig']['fsvc:hasFileChannel'];
      if (envFileChannel && typeof envFileChannel === 'object') {
        ...
        console.log(
          '    - Retention Days:',
          envFileChannel['fsvc:logRetentionDays'],
        );
        console.log('    - Max Files:', envFileChannel['fsvc:logMaxFiles']);
        console.log(
          '    - Max File Size:',
          envFileChannel['fsvc:logMaxFileSize'],
        );
        console.log(
          '    - Rotation Interval:',
          envFileChannel['fsvc:logRotationInterval'],
        );
      }
    }
    ...
    const mergedFileChannel =
      mergedConfig['fsvc:hasLoggingConfig']['fsvc:hasFileChannel'];

    console.log('✅ Merged configuration:');
    console.log(
      '  - Retention Days:',
      mergedFileChannel['fsvc:logRetentionDays'],
    );
    console.log('  - Max Files:', mergedFileChannel['fsvc:logMaxFiles']);
    console.log('  - Max File Size:', mergedFileChannel['fsvc:logMaxFileSize']);
    console.log(
      '  - Rotation Interval:',
      mergedFileChannel['fsvc:logRotationInterval'],
    );
    ...
    const invalidFileChannel2 = invalidConfig2['fsvc:hasLoggingConfig']
      ?.['fsvc:hasFileChannel'];

    if (invalidFileChannel2 && typeof invalidFileChannel2 === 'object') {
      const hasRotationInterval = 'fsvc:logRotationInterval' in
        invalidFileChannel2;
      console.log(
        '✅ Invalid rotation interval rejected:',
        !hasRotationInterval,
      );
    }
