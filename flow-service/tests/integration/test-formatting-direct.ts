...
const testCases = [
  {
    ...
    context: { operation: 'startup' as const, component: 'config-resolver' as const }
  },
  {
    level: 'WARN' as LogLevel,
    message: 'Configuration override detected',
    context: { operation: 'config-resolve' as const, configSource: 'environment' as const }
  },
  {
  ...
  },
  {
    ...
    context: { operation: 'scan' as const, meshPath: '/data/mesh', nodeCount: 42, duration: 1250 }
  }
];

console.log('📝 Writing pretty formatted logs...');
for (const testCase of testCases) {
  const formatted = formatPrettyMessage(testCase.level, testCase.message, testCase.context);
