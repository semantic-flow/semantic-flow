...
import { DEFAULT_CONTEXT } from "../defaults.ts";
...
//TODO: construct a proper service config object
export function loadEnvConfig(): ServiceConfigInput {
  ...
  if (hasLoggingConfig) {
    loggingConfig["@id"] = "_service-config/loggingConfig";
  ...
  if (hasContainedServices) {
    containedServices["@id"] = "_service-config/containedServices";
  ...
  if (hasNodeDefaults) {
    nodeDefaults["@id"] = "_service-config/meshRootNodeConfig";
    nodeDefaults["@type"] = "conf:MeshRootNodeConfig";
