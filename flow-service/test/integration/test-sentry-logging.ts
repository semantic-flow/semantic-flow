...
async function testSentryLogging() {
  ...
  try {
    ...
    console.log('🔍 Sending INFO message (should NOT go to Sentry - below warn level)');
    ...
    console.log('⚠️  Sending WARN message (should go to Sentry - at warn level)');
    ...
    console.log('❌ Sending ERROR message (should go to Sentry - above warn level)');
    await logger.error('Test error message for log retention config',
    ...
    await contextualLogger.error('Log retention configuration test error',
